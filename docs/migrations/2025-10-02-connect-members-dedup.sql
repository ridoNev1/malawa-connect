-- Patch: Deduplicate connections in get_members_org5 (Org 5)
-- Avoid duplicate member rows when multiple connection records exist
-- (e.g., after unfriend then send request again).
...
BEGIN
  -- Determine base location for 'nearest' if not provided
  ...
  WITH base AS (
    ...
    WHERE c.organization_id = 5
      ...
      -- IMPORTANT: do not filter nearest here; filter after joining presence
      AND NOT EXISTS (
  ...
  -- Deduplicate by the latest record per peer
  conns AS (
